...
1. `npm run seed:demo` — noter les identifiants et l'URL de suivi affichés en
   fin de script (le jeton change à chaque seed).
2. `.env` : `SMTP_HOST=localhost`, `SMTP_PORT=1025` (sans `SMTP_USER` — le
   transport n'envoie pas de bloc d'authentification, c'est prévu pour
   Mailpit). Lancer Mailpit, vérifier http://localhost:8025.
3. `npm run dev` — vérifier http://localhost:3000.
...
| 19:00 | candidature fraîche | Accepter : remplir le contrat, **dessiner la signature au pad**, valider | « Le PDF est généré, horodaté, et son empreinte SHA-256 est calculée — c'est la preuve d'intégrité. » | Métier, PDF, crypto |
| 20:30 | même écran | Envoyer l'invitation à signer | « Le candidat reçoit un lien de contre-signature. » | Workflow |
| 21:00 | Mailpit → lien de suivi | Ouvrir la page de signature candidat, cocher l'acceptation, signer, valider | « Ouvrir un lien de suivi neutralise la session école du navigateur — anti-usurpation. Le PDF final porte les deux signatures et une nouvelle empreinte. » | Signature, sécurité |
